...
const OfferLetter = React.forwardRef(({ formData = {} }, ref) => {
  ...
  const formatNumber = (value) => {
    ...
    // Convert value to number, remove decimal places, and use toLocaleString for commas
    return parseFloat(value.replace(/,/g, '') || '0').toLocaleString('en-IN', { maximumFractionDigits: 0 });
  };
  ...
  <div className="header-right">
    <img src="https://mrvitsolutions.com/wp-content/uploads/2024/06/cropped-MRV_page-0001.jpg" alt="Page" />
      ...
      <div className="content">
        <h3>Offer: {formData.position}<span style={{position:'absolute',left:"630px"}}>Date:{formatDate(date)}</span></h3>
     <b>  <p style={{ textTransform: 'capitalize' }}>Ref: MRV/{formData.ref}/{formData.location}/{getPositionReference()}</p></b>
       ...
       <h2><p>Dear <strong style={{textTransform:'capitalize'}}>{formData.name},</strong></p></h2> 
        ...
        <p>We are suitably impressed with your credentials and feel that your working with us will be mutually
          ...
          Annual Compensation Package including all benefits will be <strong> Rs. {formatNumber(formData.salary)}/-</strong> per annum. Annexure 1
          provides a break-up of the compensation package.</p>
        <p style={{fontWeight:'bold',fontSize:'19px'}}>You are required to join us from <strong>{formatDate(joiningDate)}</strong></p>
    ...
    <img src="https://mrvitsolutions.com/wp-content/uploads/2024/06/cropped-MRV_page-0001.jpg" alt="Page" />
        ...
        <div className="content">
          ...
          <p><strong>Basic Salary:</strong><br />
            Your Basic Basic will be <strong> Rs. {formatNumber(formData.basicMonthly)}/- </strong>per month.<br />
            Your HRA will be <strong> Rs. {formatNumber(formData.hraMonthly)}/-</strong> per month.</p>
  ...
  <div className="header-right">
    <img src="https://mrvitsolutions.com/wp-content/uploads/2024/06/cropped-MRV_page-0001.jpg" alt="Page" />
        ...
        <div className="content">
          ...
          <p>DESIGNATION: <strong> {formData.position}</strong><span style={{marginLeft:'340px'}}>DOJ: <strong> {formatDate(joiningDate)}</strong></span></p>
          <br></br>
      ...
      <tbody>
        <tr>
          <td>BASIC</td>
          <td style={{ textAlign: 'right' }}>₹{formatNumber(formData.basicMonthly)}</td>
          <td style={{ textAlign: 'right' }}>₹{formatNumber(formData.basicAnnual)}</td>
        </tr>
        <tr>
          <td>HRA</td>
          <td style={{ textAlign: 'right' }}>₹{formatNumber(formData.hraMonthly)}</td>
          <td style={{ textAlign: 'right' }}>₹{formatNumber(formData.hraAnnual)}</td>
        </tr>
        <tr>
          <td>PROFESSIONAL ALLOWANCE</td>
          <td style={{ textAlign: 'right' }}>₹{formatNumber(formData.paMonthly)}</td>
          <td style={{ textAlign: 'right' }}>₹{formatNumber(formData.paAnnual)}</td>
        </tr>
        <tr>
          <td>CONVEYANCE</td>
          <td style={{ textAlign: 'right' }}>₹{formatNumber(formData.convMonthly)}</td>
          <td style={{ textAlign: 'right' }}>₹{formatNumber(formData.convAnnual)}</td>
        </tr>
        <tr>
          <td>OTHER ALLOWANCE</td>
          <td style={{ textAlign: 'right' }}>₹{formatNumber(formData.medMonthly)}</td>
          <td style={{ textAlign: 'right' }}>₹{formatNumber(formData.medAnnual)}</td>
        </tr>
        <tr>
          <td><strong>GROSS SALARY (A)</strong></td>
          <td style={{ color: 'blue', textAlign: 'right' }}>₹{formatNumber(formData.grossMonthly)}</td>
          <td style={{ color: 'blue', textAlign: 'right' }}>₹{formatNumber(formData.grossAnnual)}</td>
        </tr>
        <tr>
          <td colSpan={3}></td>
        </tr>
        ...
        <tr>
          ...
          <td style={{ textAlign: 'right' }}>₹{formatNumber(formData.ptaxMonthly)}</td>
          <td style={{ textAlign: 'right' }}>₹{formatNumber(formData.ptaxAnnual)}</td>
        </tr>
        <tr>
          <td>LESS: PF</td>
          <td style={{ textAlign: 'right' }}>₹{formatNumber(formData.pfMonthly)}</td>
          <td style={{ textAlign: 'right' }}>₹{formatNumber(formData.pfAnnual)}</td>
        </tr>
        <tr>
          <td><strong>NET PAY (A-C)</strong></td>
          <td style={{ color: 'blue', textAlign: 'right' }}>₹{formatNumber(formData.netPayMonthly)}</td>
          <td style={{ color: 'blue', textAlign: 'right' }}>₹{formatNumber(formData.netPayAnnual)}</td>
        </tr>
      </tbody>
    </table>
  ...
  <div className="header-right">
    ...
    <p>
      ...
      <p>
            ...
            Offer and intend to join service on __________________. <br /> <p style={{ textTransform: 'capitalize' }}>Name:<span style={{fontWeight:'bold'}}> {formData.name}</span></p> <br />
...
const Offer = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setTableData(data.sort((a, b) => b.id - a.id)); // Assuming data from the backend is an array of objects
    } catch (error) {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: `Offer_Letter_${currentRecord.name}`
  });
  ...
  const verifyPasscode = async () => {
    ...
    try {
      const response = await fetch('http://localhost:9000/verifypass', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${storedToken}`,
  ...
  const confirmDelete = async () => {
    ...
    try {
      await fetch(`https://mrv1.indianwelfarefoundation.org.in/delete-record/${deleteRecord.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${storedToken}`,
      ...
      setTableData(tableData.filter(record => record.id !== deleteRecord.id));
      setShowModal(false);
      setIsVerified(false);
      setPasscode('');
      setError('');
...
navigate('/pdf')
  ...
  const handleEdit = (record) => {
    localStorage.setItem('editoffer', JSON.stringify(record));
    navigate('/Editoffer'); // Replace with your actual edit page route
  };
  ...
  const handleCloseModal = () => {
    ...
    setIsVerified(false)
    setDel(false)
    // Reset state variables when closing modal
  };
  return (
    <div>
      <NavbarMain />

      <button onClick={nav} style={{ marginLeft: '20px', alignItems: 'start', marginTop: '20px', padding: '15px', textTransform: 'capitalize' }}>
        Go to Generate Offer Letter
      </button>
        ...
        <table className="styled-table" style={{ textTransform: 'capitalize', textAlign: 'center', padding: "20px" }}>
          <thead style={{ textAlign: 'center' }}>
            <tr style={{ textAlign: 'center' }} className='tb'>
              ...
              <th style={{ padding: '8px', textAlign: 'center' }}>Job Location</th>
              <th colSpan={3} style={{ padding: '8px', textAlign: 'center' }}>Action</th>
            </tr>
          </thead>
          <tbody>
            {filteredData.slice(indexOfFirstRecord, indexOfLastRecord).map((item, index) => (
              <tr key={index}>
                <td style={{ padding: '8px', textAlign: 'center' }}>{item.date.split('T')[0].split('-').reverse().join('-')}</td>
      ...
      {showModal && (
        <div className="modal">
          <div className="modal-content">
            ...
            <h3 style={{ textAlign: 'center' }}>Delete Offer Letter</h3>
            <p>Reference: {deleteRecord.ref}</p>
         ...
         <button className='ver' onClick={verifyPasscode} disabled={isVerified}
              style={{ backgroundColor: isVerified ? 'green' : 'blue', color: 'white' }}>
              {load ? (
            ...
            {del ? (
              <p className="rec" onClick={confirmDelete}>Delete</p>
            ) : (
              <p></p>
            )}
          </div>
